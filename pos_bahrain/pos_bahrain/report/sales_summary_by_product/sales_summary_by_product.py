...
from pos_bahrain.utils import pick, split_to_list, with_report_error_check, key_by
from pos_bahrain.api.sales_invoice import get_payments_against
...
def _get_filters(filters):
    ...
    si_clauses = join_clauses(
        ["si.docstatus = 1"], ["si.pb_branch IN %(branches)s"] if branches else [],
    )
    ...
    items = frappe.db.sql(
        """
            SELECT
                ...
                sii.pb_minimum_selling_rate AS ms1,
                IF(
                    (sii.amount / sii.qty) < sii.pb_minimum_selling_rate,
                    'Yes',
                    'No'
                ) AS below_ms1,
                sii.pb_minimum_selling_2_rate AS ms2,
                IF(
                    (sii.amount / sii.qty) < sii.pb_minimum_selling_2_rate,
                    'Yes',
                    'No'
                ) AS below_ms2,
