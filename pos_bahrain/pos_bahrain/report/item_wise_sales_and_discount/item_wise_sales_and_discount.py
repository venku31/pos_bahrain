...
def get_items(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			...
			`tabSales Invoice Item`.`item_name`, `tabSales Invoice Item`.`item_group`,`tabSales Invoice Item`.`brand`,
			`tabSales Invoice Item`.sales_order, `tabSales Invoice Item`.delivery_note,
			`tabSales Invoice Item`.income_account, `tabSales Invoice Item`.cost_center,
			`tabSales Invoice Item`.stock_qty, `tabSales Invoice Item`.stock_uom,`tabSales Invoice Item`.base_price_list_rate as price_list_rate,`tabSales Invoice Item`.discount_amount,
			`tabSales Invoice Item`.base_net_rate, `tabSales Invoice Item`.base_net_amount,`tabSales Invoice Item`.discount_percentage,
			`tabSales Invoice`.customer_name, `tabSales Invoice`.customer_group, `tabSales Invoice Item`.so_detail,
			(select default_supplier from `tabItem Default` where parent=`tabSales Invoice Item`.item_code and company=`tabSales Invoice`.company) as default_supplier,
