		...
		if item_map.get(item):
			...
			if batch_no:
				...
				if batch_doc:
					...
					expiry_in_days = ((batch_doc.expiry_date - getdate()).days if batch_doc.expiry_date else None)
...
def get_stock_ledger_entries(filters, items):
    ...
    sql_query = """
        select
            ...
            sle.item_code as name, sle.voucher_no, sle.stock_value, sle.batch_no, sle.actual_qty  
        from
            `tabStock Ledger Entry` sle force index (posting_sort_index)
        where sle.docstatus < 2 %s %s
        order by sle.posting_date, sle.posting_time, sle.creation, sle.actual_qty
    """ % (conditions, item_conditions_sql)
...
def get_item_warehouse_map(filters, sle):
	...
	for d in sle:
		...
		qty_dict.batch_quantity += d.actual_qty
